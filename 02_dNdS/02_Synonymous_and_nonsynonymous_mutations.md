...
\
\
I first map PacBio long reads from _B. odoriphaga_ to the _B. coprophila_ genome with the autosome and X, then call variants. The _B. odoriphaga_ reads can be found under accession SRR11366020.
```
# samtools (v1.21)
# minimap2 (v2.28)
# picard (v3.3.0)
...
# Eefine file names, including the long reads for B. odoriphaga, and the reference genome for B. coprophila
...
# I have then created a data/ folder in the same directory. Within that I have created a Bradysia_coprophila directory and copied in both the ref genome and the gff3 annotation
...
snpEff build -gff3 -v Bradysia_coprophila
